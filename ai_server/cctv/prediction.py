...
# 데이터 및 라벨 로드
data = np.load('origin_30fps.npy')  
print(f"{data.shape}")
with open('combined_label_10s.pkl', 'rb') as f:
...
model.load_state_dict(torch.load('/root/install_stgcn/st-gcn/work_dir/shuffle_combined_model_10s/epoch200_model.pt', map_location=device))
